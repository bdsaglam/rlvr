...
```sh
uv pip install -e environments/vf_musique
uv run rl @ configs/prime-rl/musique.toml
```

Notes:
- Run from the repo root so `prime env install vf-musique` can discover `./environments/vf_musique`.
- To debug quickly, edit `configs/prime-rl/musique.toml` and swap `datasets_str` to `bdsaglam/musique-mini,answerable,train`.
- If you use a chat template that strips past thinking (e.g., Qwen3), set `--orchestrator.trajectory-strategy branching`.
