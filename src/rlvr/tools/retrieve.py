...
def make_bm25_retriever(
    ...
    def retrieve(docs: list[Document], query: str) -> list[Document]:
        ...
        Args:
            ...
            top_n: Number of documents to retrieve (default: 3)
...
def make_rerank_retriever(
    ...
    kwargs: dict[str, Any] = dict(top_n=top_n, return_documents=False)
...
def combine_retrieval_results(
    ...
    for ranked_docs in ranked_docs_list:
        ...
        # Add top_n + 1 to documents not retrieved by this retriever
        for doc in docs:
            if doc["id"] not in seen:
                combined_ranks[doc["id"]] += top_n
    ...
    return [docs_mapping[id] for id, _ in sorted_results[:top_n]]
...
def make_combined_retriever(
    *retrievers: list[Callable],
    ...
    def retrieve(docs: list[Document], query: str) -> list[Document]:
        ...
        return combine_retrieval_results(ranked_docs_list, docs, top_n)
...
def make_retrieve_tool(name: str = "lexical", top_n: int = 3) -> Callable:
    if name == "golden":
        retriever = golden_retriever
    elif name == "bm25":
        retriever = make_bm25_retriever(top_n=top_n)
    elif name == "hybrid":
        ...
        retriever = make_rerank_retriever(top_n=top_n, model="bm25")
